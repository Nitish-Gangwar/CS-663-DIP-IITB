function reconstructed_image=myPCADenoising2(im2_patch)
sigma_noise=20;
im2_noise = double(im2_patch) + double(randn(size(im2_patch))*sigma_noise);

%figure,imshow(uint8(im2_noise));
...
for i=1:(imagesize(1)-neighbourhood+1)
    for j=1:(imagesize(2)-neighbourhood+1)
        ...
        qi=similarpatches(centermost_patch,wholewindow);
